...
class SocketHandler:
    ...
    'accept-language': 'en-US,en;q=0.9',
        ...
        response = requests.get('https://aminoapps.com/api/chat/web-socket-url', headers=header, cookies=cookies)
        response=json.loads(response.text)
        url=response['result']['url']
        return url

    async def run(self):
        header = {"cookie": f"sid={self.client.sid};"}

        async with websockets.connect(await self.get_websocket_url(),extra_headers=header) as websocket:
